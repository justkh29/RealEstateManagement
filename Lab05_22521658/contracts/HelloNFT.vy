...
ownerToNFTTokenCount: HashMap[address, uint256]
...
def balanceOf(_owner: address) -> uint256:
    ...
    return self.ownerToNFTTokenCount[_owner]
...
def _addTokenTo(_to:address, _tokenId: uint256):
    ...
    self.ownerToNFTTokenCount[_to] += 1

@internal 
def _removeTokenFrom(_from: address, _tokenId: uint256):
    assert self.idToOwner[_tokenId] == _from
    self.idToOwner[_tokenId] = empty(address)
    self.ownerToNFTTokenCount[_from] -= 1
...
def _transferFrom(_from: address, _to: address, _tokenId: uint256, _sender: address):
    ...
    log Transfer(_from=_from, _to=_to , _tokenId=_tokenId)
...
def approve(_approved: address, _tokenId: uint256):
    ...
    log Approval(_owner=owner, _approved=_approved, _tokenId=_tokenId)
