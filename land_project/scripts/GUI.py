...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        try:
            # 1. Lấy provider từ hệ thống mạng của Ape
            # Chúng ta muốn kết nối vào mạng 'ethereum:local' dùng provider 'test' (hoặc 'geth')
            # Cách an toàn nhất là dùng .use_provider để lấy context, sau đó vào sâu bên trong
            
            self.provider_context = networks.ethereum.local.use_provider(NODE_URL)
            
            # 2. Kích hoạt context thủ công (__enter__)
            # Điều này tương đương với việc bắt đầu khối 'with'
            self.active_provider = self.provider_context.__enter__()
            
            print(f"✅ Đã kết nối đến Blockchain tại {NODE_URL}")
            
        except Exception as e:
            QMessageBox.critical(self, "Lỗi Mạng", f"Không thể kết nối Node: {e}")
            # Có thể thoát app nếu không có mạng
            # sys.exit(1)
    ...
    def closeEvent(self, event):
        """Hàm này được gọi khi cửa sổ đóng lại."""
        if hasattr(self, 'provider_context'):
            print("Đang ngắt kết nối mạng...")
            # Kết thúc context (__exit__)
